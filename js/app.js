...
function bindAccountCenter() {
  ...
  const btnCheckUpdate = $('#me-check-update');
  ...
  if (btnCheckUpdate) btnCheckUpdate.onclick = async () => {
    ...
    const ot = btnCheckUpdate.textContent;
    btnCheckUpdate.textContent = '检查中…';
    try {
      if (!reg || !reg.update) { toast('当前浏览器未启用 Service Worker'); return; }
      await reg.update();
      // 有新版本的话，SW updatefound 监听到会弹 toast「点击刷新」
      const nw = reg.installing || reg.waiting;
      if (nw) {
        toast('🚀 已下载新版本，点击 toast 顶部提示立即刷新应用', 6000);
      } else {
        toast('✅ 已是最新版本');
      }
    } catch (e) {
      toast('检查更新失败：' + String(e?.message || e));
    } finally {
      btnCheckUpdate.disabled = false;
      btnCheckUpdate.textContent = ot;
    }
  };
